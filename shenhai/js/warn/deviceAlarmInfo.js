...
myApp.controller('customersCtrl',function($scope,$http,ngDialog,$modal,$timeout){
    ...
    var ModalInstanceCtrl = function ($scope, $modalInstance,$http,ngDialog) {
    	...
    	$http({
    		 method:'POST',
    		 url:'getDeviceAlarmConfigList.do',
    		 params:tData})
    		 .success(function(response){
    			 $scope.configList = response;
    			 if(mtype==1){
    				 $scope.station.configId=$scope.configList[0].id;
    			 }
    		 });
    	...
    	$scope.save = function(){
    		$scope.station.beginTime = document.getElementById("beginTime").value;
    		$scope.station.endTime = document.getElementById("endTime").value;
