...
myApp.controller('customersCtrl',function($scope,$http,ngDialog,$modal,$timeout){
	...
	$scope.dele = function(){
		...
		if(station==null){
			alert("请选择要删除的站点设备维护配置");
			return;
		}
		var dParam = station;
		$http({
			 method:'POST',
			 url:'deleStationDeviceMain.do',
			 params:dParam})
    ...
    $scope.query=function(){
    	...
    	$http({
    		 method:'POST',
			 url:'showStationDeviceMainList.do',
			 params:pData})
			 .success(function(response){
    ...
    var ModalInstanceCtrl = function ($scope, $modalInstance,$http,ngDialog) {
    	...
    	$http({
    		 ...
    		 .success(function(response){
    			 $scope.stationList = response;
    			 if(mtype==1){
    				 $scope.station.stationId=$scope.stationList[0].id;
    			 }else if(mtype==2){
    	...
    	//根据站点和设备得到站点设备配置ID
    	$scope.getMadcId = function(){
    		var mData={
    				stationId:$scope.station.stationId,
    				deviceId:$scope.station.deviceId
    		};
    		$http({
          		 method:'POST',
          		 url:'getStationDeviceByWidDid.do',
          		 params:mData})
          		 .success(function(response){ 
          			$scope.station.madcId = response.id;
          		 });
    	};
    	...
    	$http({
     		 method:'POST',
     		 url:'getAiotMainConfigList.do',
     		 params:aData})
     		 .success(function(response){ 
     			$scope.amcList = response;
     			if(mtype==1){
     				$scope.station.amconfigId = $scope.amcList[0].id;
     			}else{
     				$scope.station.amconfigId = station.amconfigId;
     			}
     	});
    	...
    	$scope.save = function(){
    		...
    		if(mtype==2){
    			 $http({
        	   		 method:'POST',
        				 url:'saveChangeStationDeviceMain.do',
        				 params:saveparam,
