    	...
    	$http({
	   		 method:'POST',
				 ...
				 .success(function(response){
			    	 ...
			    	 if(mtype==1){
			    		 $scope.group.item = $scope.indicatorlist[0].code;
			    	 }
	   	}); 
    	$scope.minlist=[{id:0,value:'>'},{id:1,value:'>='}];
    	$scope.maxlist=[{id:0,value:'<'},{id:1,value:'<='}];
    	if(mtype==1){
      		 $scope.group.min = $scope.minlist[1].id; 
      		 $scope.group.max = $scope.maxlist[0].id; 
      	}
    	//获得水质等级名称列表,并初始化
    	$scope.getLevelList=function(){
    		var levelparam = {
    				typeId:$scope.group.waterType
    		};
    		$http({
    			method:'POST',
    			url:'getWaterStandardConfigList.do',
    			params:levelparam,
    			headers: { 'Content-Type': 'application/json; charset=UTF-8'}
    		})
    		.success(function(response){
    			...
    			if(mtype ==1){
		    		 $scope.group.standardId =  $scope.levelList[0].classId;
		    	 }
    		});
	   	}; 
    	...
    	$http({
	   		 method:'POST',
				 ...
				 .success(function(response){
			    	 ...
			    	 if(mtype ==1){
			    		 $scope.group.waterType =  $scope.typelist[0].classId;
			    	 }
			    	 $scope.getLevelList();
	   	}); 
